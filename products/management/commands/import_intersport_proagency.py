...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		nb_proagency = 0
		# Read file data
		f = urlopen("http://eloue.s3.amazonaws.com/intersport_agencies.xlsx")
		...
		for x in range(14, sh.nrows):
			...
			try:
				phone_number = str(int(sh.row(x)[4].value))
				phone_number = '0' + phone_number
			except Exception, e:
				pass
			
			try:
				newAgency = ProAgency.objects.create(name=name, address1=address, zipcode=zipcode, phone_number=phone_number, patron_id=patron_id)
				nb_proagency += 1
			except Exception, e:
				raise e
