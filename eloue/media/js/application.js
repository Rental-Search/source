...
if ($("input[name$='l']")){    
	...
	//alert("-2 enter navigator");
        navigator.geolocation.getCurrentPosition(function(position){
		//alert("-1 enter navigator");
				...
				for(i = 0 ; i <= results[0].address_components.length-1 ; i++){						
					if(results[0].address_components[i].types[0] == "locality"){
						...
						if ($("input[name$='l']").val() == ''){						   	
							...
							//alert("first user_location  0 "+user_location);
						}
						//$("input[name$='l']").val(results[0].address_components[i].long_name);
						break;
					}
	...
	$("input[name$='l']").change(function(){
		if ($("input[name$='l']").val() != ''){
			user_location=$("input[name$='l']").val();
			$("input[name$='l']").val(user_location);
     			//alert("changed user_location : 1 "+user_location);
			//alert("second : "+$("input[name$='l']").val());
		}
	});
	
	});
	//alert("default value 2 " + user_location);
	//$("input[name$='l']").val(user_location);
