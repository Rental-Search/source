...
from itertools import imap, chain, count

class Migration(DataMigration):

    def creator(self, orm, name, root):
        postfix = imap(lambda n: '-'+str(n), count(1))
        category = orm.Category(parent=root, name=name)
        slug = slugify(name)
        while orm.Category.objects.filter(slug=slug):
            slug = slugify(name) + postfix.next()
        else:
            category.slug = slug
            category.save()
    ...
    def forwards(self, orm):
        ...
        automobile.save()
        automobile.sites.add(*settings.DEFAULT_SITES)
        names = [
            u'Petite citadine', u'Compacte', u'Berline', u'Break', u'Monospace, ludospace',
            u'Cabriolet', u'Coupé', u'4x4, Pickup, Crossover', u'Minibus', u'Utilitaire léger < 2t',
            u'Ancien, Collection', u'Autre'
        ]
        for name in names:
        ...
        names = [
            u'Appartement', u'Maison', u'Bed & Breakfast', u'Cabane', u'Villa', 
            u'Château', u'Chambre', u'Cabane dans les arbres', u'Bateau', u'Avion', 
            u'Caravane', u'Igloo', u'Phare', u'Yourte', u'Tipi', u'Cave', u'Île', u'Chalet',
            u'Maison de terre', u'Hutte', u'Train', u'Tente', u'Loft', u'Autre', u'Chambre d’ami'
        ]
        for name in names:
    ...
    models = {
        ...
        'products.category': {
            'Meta': {'ordering': "['name']", 'object_name': 'Category','_bases': ('mptt.models.MPTTModel', )},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
