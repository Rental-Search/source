...
class PatronEngagementScoreCalculator(object):
    def calculate_user_engagement_score(self, patron, start_date, end_date):
        ...
        period_fee_total = sum([booking.fee for booking in Booking.objects.filter(created_at__gte=start_date, created_at__lte=end_date).filter(Q(owner=patron) | Q(borrower=patron))])
