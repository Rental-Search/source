...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        def rib_to_iban(rib):
            # http://fr.wikipedia.org/wiki/Basic_Bank_Account_Number#Conversion_du_RIB_en_IBAN_en_Python
            tmp_iban = int("".join([str(int(c, 36)) for c in rib + "FR00"]))
            key = 98 - (tmp_iban % 97)
            return "FR%.2d%s" % (key, rib)

        for patron in orm.Patron.objects.exclude(rib=''):
            patron.iban = rib_to_iban(patron.rib)
            try:
                patron.clean_fields()
