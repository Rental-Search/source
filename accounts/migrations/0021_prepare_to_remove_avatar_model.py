...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for avatar in orm['accounts.avatar'].objects.all():
            patron = avatar.patron
            # we use patron.patron, because avatar.patron points to User
            patron.patron.avatar = avatar.image
            patron.patron.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for patron in orm['accounts.patron'].objects.filter(~models.Q(avatar=None), ~models.Q(avatar='')):
            try:
                patron.avatar_old.image = patron.avatar
            except orm['accounts.avatar'].DoesNotExist:
                patron.avatar_old = orm['accounts.avatar'](image=patron.avatar, created_at=datetime.datetime.now())
            finally:
                patron.avatar_old.save()
                patron.avatar = ''
                patron.save()
