...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'OwnerComment', fields ['booking']
        db.delete_unique(u'rent_ownercomment', ['booking_id'])

        # Changing field 'OwnerComment.booking'
        db.alter_column(u'rent_ownercomment', 'booking_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['rent.Booking']))

        # Renaming model 'OwnerComment' to 'Comment'
        db.rename_table('rent_ownercomment', 'rent_comment')

        # Adding field 'Comment.type'
        db.add_column(u'rent_comment', 'type',
                      self.gf('django.db.models.fields.PositiveSmallIntegerField')(default=0, db_index=True),
                      keep_default=False)

        # Note: we will handle BorrowerComment after we have moved it's data into
        # Comment in the following data migration

    def backwards(self, orm):

        # Note: BorrowerComment data must have been already moved to the separate table

        # Renaming model 'Comment' to 'OwnerComment'
        db.rename_table('rent_comment', 'rent_ownercomment')

        # Deleting field 'OwnerComment.type'
        db.delete_column(u'rent_ownercomment', 'type')

        # Changing field 'OwnerComment.booking'
        db.alter_column(u'rent_ownercomment', 'booking_id', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['rent.Booking'], unique=True))

        # Adding unique constraint on 'OwnerComment', fields ['booking']
        db.create_unique(u'rent_ownercomment', ['booking_id'])
